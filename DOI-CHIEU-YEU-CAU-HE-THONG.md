...
| Desktop app, Monitoring app, Web browser, Mobile iOS/Android | ⚠️ | Có **Web client** (`emap-demo`, chạy trên Chrome/Edge/Firefox). **Desktop client** đang được xây lại (`wata-desktop-client`, bản WPF cũ đã bị xoá) bằng **Avalonia UI (.NET 8)** để đa nền tảng — đã có đăng nhập thật, bố cục 3 cột giống web, lọc theo Zone, playback/bookmark, bản đồ (Mapsui), panel Quản lý người dùng/ANPR/Khuôn mặt/Hành vi. **Chưa có**: panel VCA, hardware decode (software decode qua FFmpeg.AutoGen). Xem `wata-desktop-client/README.md`. Chưa có Monitoring app riêng, chưa có app Mobile iOS/Android |
...
3. **Desktop client đang được làm lại** (`wata-desktop-client`, Avalonia UI .NET 8 — bản WPF cũ đã bị xoá trước đó) — đã có đăng nhập, bố cục giống web, Zone/Users/Playback/Bookmark, bản đồ, panel ANPR/Khuôn mặt/Hành vi; còn thiếu VCA, hardware decode, multi-monitor/PTZ HID mô tả trong spec. Xem roadmap trong `wata-desktop-client/README.md`.
