...
| Desktop app, Monitoring app, Web browser, Mobile iOS/Android | ⚠️ | Có **Web client** (`emap-demo`, chạy trên Chrome/Edge/Firefox). **Desktop client** đang được xây lại (`wata-desktop-client`, bản WPF cũ đã bị xoá) bằng **Avalonia UI (.NET 8)** để đa nền tảng — đã có đăng nhập thật, bố cục 3 cột giống web, lọc theo Zone, playback/bookmark, bản đồ (Mapsui), panel Quản lý người dùng/ANPR/Khuôn mặt/Hành vi/VCA (vẽ line/vùng đè lên video). **Chưa có**: hardware decode (software decode qua FFmpeg.AutoGen), grid nhiều camera, đa màn hình, PTZ/HID. Xem `wata-desktop-client/README.md`. Chưa có Monitoring app riêng, chưa có app Mobile iOS/Android |
...
⚠️ Khác cách tiếp cận — hệ thống dùng bản đồ địa lý thực (OpenStreetMap) với marker camera/zone, **không phải** upload ảnh sơ đồ mặt bằng (JPG/PNG/BMP tới 8.25MP) như spec mô tả. Web (`emap-demo`, OpenLayers) đã có đầy đủ nghiệp vụ tương tác (click xem camera, cảnh báo realtime, heatmap, tìm bán kính). Desktop (`wata-desktop-client`, Mapsui) mới có marker camera + polygon zone (click để chọn/lọc) — chưa có heatmap, tìm bán kính, vẽ zone mới, vẽ lộ trình biển số.
...
3. **Desktop client đang được làm lại** (`wata-desktop-client`, Avalonia UI .NET 8 — bản WPF cũ đã bị xoá trước đó) — đã có đăng nhập, bố cục giống web, Zone/Users/Playback/Bookmark, bản đồ, panel ANPR/Khuôn mặt/Hành vi/VCA; còn thiếu hardware decode, multi-monitor/PTZ HID mô tả trong spec. Xem roadmap trong `wata-desktop-client/README.md`.
